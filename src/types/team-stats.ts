/**
 * Team stats data type definitions
 * @module types/teamstats
 */

import type { Season, TeamAbbr, Week } from './common';
import type { SummaryLevel } from './player-stats';

/**
 * Team statistics record
 * Matches the nflverse `stats_team` release schema (nflfastR::calculate_stats),
 * which aggregates official box score statistics to the team level.
 *
 * Week/game fields (week, season_type, opponent_team) are only present in
 * week-level files; season-level files (reg, post, reg+post) omit them.
 */
export interface TeamStatsRecord {
  ...
  /** Sack yards recorded by the defense */
...
/**
 * Array of team stats records
 */
export type TeamStatsData = TeamStatsRecord[];
